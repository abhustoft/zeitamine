...
class House extends Component {
    componentDidMount() {
        ...
        buildings
            ...
            .staggerFrom(build1wind, 1.5, {y: -80, ease: Elastic.easeOut}, 0.2,    '-=1.3')
            .from('.building2',      1.5, {y:  80, ease: Elastic.easeOut},         '-=2.2')
            .staggerFrom(build2wind, 1.5, {y: -80, ease: Elastic.easeOut}, 0.2,    '-=1.5')
            .from('.building4',      1.5, {y:  90, x: -10, ease: Elastic.easeOut}, '-=3.3')
            .staggerFrom(build4wind, 1.5, {y: -80, ease: Elastic.easeOut}, 0.2,    '-=1.3')
            .from('.building3',      1.5, {y:  80, x: 10, ease: Elastic.easeOut},  '-=4.5')
            .staggerFrom(build3wind, 1.5, {y: -80, ease: Elastic.easeOut}, 0.2,    '-=2.4')
            .from('.walls',    0.2, {y: 80,  ease: Elastic.noease},                '-=2.7')
            .from('.door',     0.1, {scaleX: 0,  ease: Elastic.noease, transformOrigin: 'bottom left'}, '-=2.5')
            .from('.doorway',  0.1, {scale: 0}, '-=0.1')
            .from('.window',   0.2, {scale: 0,  ease: Elastic.noease}, '-=2.4')
            .from('.roof',     1.3, {y: -80, ease: Bounce.easeOut},    '-=2.5')
            .from('.chimney',  0.3, {y: -80, ease: Power0.easeOut},    '-=1.4')
            .from('.doorknob', 1.5, {y: 50,  ease: Elastic.easeOut},   '+=.5')
            .set('.bush', {x: '85%', y: '100%'}, '-=2.9')
            .fromTo('.bush',   1.0, {scale: 0}, {ease: Elastic.easeOut, transformOrigin: 'bottom right', scale: 1}, '-=2.9')
            ...
            .to('.door',             2.0, {skewY: "-10deg",  ease: Elastic.noease, transformOrigin: 'bottom right'}, '+=1.5')
            .to('.doorknob',         2.0, {y: 3,  ease: Elastic.noease},          '-=2.0')
            .to('.door',             0.2, {skewY: "0deg",  ease: Elastic.noease, transformOrigin: 'bottom right'}, '+=0.5')
            .to('.doorknob',         0.2, {y: 0,  ease: Elastic.noease},          '-=0.2');
    }
